...
const EditBranchModal: React.FC<EditBranchModalProps> = ({ branch, onClose, onSave }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    settings: {
      volume: branch.settings?.volume || 50,
      operatingHours: {
        start: branch.settings?.operatingHours?.start || '09:00',
        end: branch.settings?.operatingHours?.end || '18:00'
      },
      timezone: branch.settings?.timezone || 'UTC+3'
    }
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updatedBranch = await branchService.updateBranch(branch.id, formData);
